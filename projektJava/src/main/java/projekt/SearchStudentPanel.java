...
/**
 * Klasa reprezentująca panel do wyszukiwania studentów po określonych danych
 * podanych przez użytkownika (imię, nazwisko, numer albumu, grupa).
 * <p>
 * Wyniki są wyświetlane w tabeli i filtrowane dynamicznie na podstawie danych
 * wejściowych.
 * </p>
 * 
 * @author Rafał Okoniecki
 */
public class SearchStudentPanel extends JPanel {
	...
	/**
	 * Wyszukuje studentów na podstawie wprowadzonych kryteriów i aktualizuje tabelę
	 * wyników.
	 *
	...
	private void searchStudents(String name, String surname, String albumNumber, String groupCode) {
		...
		List<Student> filteredStudents = studentDB.getStudents().stream()
				...
				.filter(student -> (surname.isEmpty()
				...
				.filter(student -> (groupCode.isEmpty() || (student.getGroup() != null
						&& student.getGroup().getGroupCode().toLowerCase().contains(groupCode.toLowerCase()))))
				.collect(Collectors.toList());

		for (Student student : filteredStudents) {
			resultTableModel.addRow(new Object[] { student.getName(), student.getSurname(), student.getAlbumNumber(),
					student.getGroup() != null ? student.getGroup().getGroupCode() : "Brak grupy" });
		}

		if (filteredStudents.isEmpty()) {
			JOptionPane.showMessageDialog(this, "Nie znaleziono studentów spełniających podane kryteria.",
