...
/**
 * Panel GUI służący do zarządzania listą studentów – umożliwia dodawanie,
 * edytowanie, usuwanie oraz podgląd wyników.
 * <p>
 * Studenci są pobierani i zapisywani za pomocą {@link StudentDB},
 * {@link GroupDB} oraz {@link SubjectDB}.
 * </p>
 * 
...
public class StudentPanel extends JPanel {
	...
	/**
	 * Wyświetla formularz i dodaje nowego studenta na podstawie danych
	...
	private void addStudent() {
		...
		if (result == JOptionPane.OK_OPTION) {
			...
			studentTableModel
					.addRow(new Object[] { name, surname, album, group != null ? group.getGroupCode() : "Brak" });
		}
	}

	/**
	 * Edytuje dane zaznaczonego studenta. Pokazuje formularz z aktualnymi danymi i
	 * zapisuje zmiany.
	 */
	private void editStudent() {
		...
		if (result == JOptionPane.OK_OPTION) {
			...
			if (newName.isEmpty() || newSurname.isEmpty()) {
				JOptionPane.showMessageDialog(this, "Imię i nazwisko nie mogą być puste!", "Błąd",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			studentTableModel.setValueAt(student.getGroup() != null ? student.getGroup().getGroupCode() : "Brak",
					selectedRow, 3);
		}
	}

	/**
	 * Usuwa zaznaczonego studenta z bazy danych i z tabeli. Pokazuje komunikat o
	 * powodzeniu.
	 */
	...
	private void showScoresForSelectedStudent() {
		int selectedRow = studentTable.getSelectedRow();
		if (selectedRow == -1) {
			JOptionPane.showMessageDialog(this, "Wybierz studenta, dla którego chcesz wyświetlić wyniki!", "Błąd",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		JDialog dialog = new JDialog(SwingUtilities.getWindowAncestor(this), "Wyniki studenta",
	...
	public void refreshStudentTable() {
		...
		for (Student s : studentDB.getStudents()) {
			studentTableModel.addRow(new Object[] { s.getName(), s.getSurname(), s.getAlbumNumber(),
